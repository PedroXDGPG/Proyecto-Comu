...
def calcular_psd(senal, samplerate):
    # Calcula la PSD utilizando la Transformada de Fourier
    psd = np.abs(np.fft.fft(senal) / len(senal))**2
    # Ajusta la PSD por el factor de escala
    psd = psd * (2 / (samplerate * len(senal)))
...
#plot_signal_vs_time(salidaTX, samplerate_resampled, duration)
